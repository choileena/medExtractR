...
Similar to `medExtractR`'s `extract_entities` subfunction, the tapering extension has its own corresponding subfunction called `extract_entities_tapering`. In addition to the entities identified by the basic `medExtractR`  function, the tapering extension can also identify: dispense amount, number of refills, dose schedule (keywords indicating a particular dose schedule pattern, such as "taper", "wean", "dose pack", or "discontinue"), time keyword (indicating whether a dose is past, current, or future), transition (words or symbols linking consecutive doses in a tapering schedule), and preposition (prepositions adjacent to other entities, used in some contexts to preserve or infer meaning). 
...
```{r}
# Taper 2 schedule
subset(pred_mxr, filename == pred_fn[2])
```
...
subset(pred_mxr, filename == pred_fn[3])
```
...
The remainder of this section outlines how `medExtractR_tapering` processes a note, from free-text input to a structured entity-level output. We'll use the following excerpt as our example input to help illustrate each step:
...
1. Weeks HL, Beck C, McNeer E, Williams ML, Bejan CA, Denny JC, Choi L. medExtractR: A targeted, customizable approach to medication extraction from electronic health records. Journal of the American Medical Informatics Association. 2020 Jan 16.  
